%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%        Segmentacja metoda projekcji wstecznej (backprojection)          %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
r_min=1; %minimalny rozwazany promien okregu
...
numpeaks=10;
okregi=zeros(0,6); % x ,y ,r ,c (ilosc wykrytych punktow)/2*pi*r

for n=1:numpeaks
   Hmax=max(H(:));
   [y,x,ri]=ind2sub(size(H),find(H==Hmax,1));
   r=ri+r_min-1;
   c=4*r;
   if Hmax>prog*c
        okregi=[okregi;[y-r_max, x-r_max, r, Hmax/c, Hmax, c]];
